...
import {ExcelJson} from "../../models/interfaces/excel-json.interface";
...
export class ExportService {
    ...
    public exportJsonToExcel(json: ExcelJson[], fileName: string): void {
        ...
        const worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(
            json[0].data,
            ExportService.getOptions(json[0])
        );
        ...
        for (let i = 1, length = json.length; i < length; i++) {
            ...
            XLSX.utils.sheet_add_json(
                worksheet,
                json[i].data,
                ExportService.getOptions(json[i], -1)
            );
        }
     ...
     */
    private static getOptions(json: ExcelJson, origin?: number): any {
        // adding actual data
        const options = {
            skipHeader: true,
            origin: -1,
     ...
     * @param separator .
     */
    public exportToCsv(rows: object[], fileName: string, columns?: string[], separator?: string): string {
        if (!rows || !rows.length) {
            return;
        }
        if(!separator) {
